...
import (
	"cloud-storage/protos/file/proto"
	"context"
	"time"
...
// DownloadPart 下载分片
func (f *FileServiceClient) DownloadPart(ctx context.Context, req *filepb.DownloadRequest) (filepb.FileService_DownloadPartClient, error) {
	// 设置默认超时时间
	if _, ok := ctx.Deadline(); !ok {
		var cancel context.CancelFunc
		ctx, cancel = context.WithTimeout(ctx, 5*time.Second)
	...
	return f.grpcClient.DownloadPart(ctx, req)
}
